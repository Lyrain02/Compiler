...
typedef struct Symbol{
    ...
    string entry;//id的名字
    string stype;//没用到
    string type;//int，real，bool
    string val;//整型和实数型 22,1.2
    string op;//操作符，bool, arith
    unordered_map<string, IdInfo> map;//局部变量的表
    Symbol(string s=""){
        symbol = s;
    }
}Symbol;
...
typedef struct SEntry{
    int stateId;//状态id
    // string symbol;//LR
    Symbol symbol;//semantic analysis
	SEntry(int num, Symbol str){
        stateId = num;
        symbol = str;
	}
    SEntry(int num){
...
bool is0(string val);//检测real型和int型(real可以调用int）

Symbol getSymbol(string input);//获取input对应的Symbol
void printResults();//输出参数结果
string standardValue(string value);//标准化real输出

string toStringSymbol(Symbol s);//转换成字符串

//-----------------------全局变量-------------------------

vector<string> variables;//存储变量名
unordered_map<string, IdInfo> idMap;//存储ids
bool isIf;//用来记录if开启
...
bool isReal(string token){
    bool meet = false;
    for(int i=0;i<token.length();i++){
        if((token[i]>='0' && token[i]<='9')||(!meet && token[i]=='.')){
            if(token[i]=='.'){
                meet = true;
            }
        }else{
...
//检测real型和int型(real可以调用int）
bool is0(string val){
    bool meet = false;
    for(int i=0;i<val.length();i++){
        if(val[i]=='0' || (!meet && val[i]=='.')){
            if(val[i]=='.'){
                meet = true;
            }
        }else{
...
Symbol getSymbol(string input){
    Symbol s;
    if(isInt(input)){
        s.symbol = "NUM";
        s.val = input;//atoi(input.c_str());
        s.type = "int";
    }else if(isReal(input)){
        s.symbol = "NUM";//atof(input.c_str());
        s.val = input;
        s.type = "real";
    }else if(isId(input)){
        s.symbol = "ID";
...
void processLR(){
    ...
    while(true){
        ...
        Symbol sym = getSymbol(input);//将input转化为symbol(5->NUM)
        SEntry top = myStack.top();
        TEntry tEntry = table[top.stateId][sym.symbol];
        ...
        if(tEntry.type=="accept"){
            ...
            myStack.push(SEntry(tEntry.stateId,sym));
            if(sym.symbol=="ID" && idMap.find(sym.entry)==idMap.end()){//存储变量
                variables.push_back(sym.entry);
                // cout<<sym.entry<<"------------"<<endl;
                idMap[sym.entry]=IdInfo();
            }            
            i++;
        }else if(tEntry.type=="reduce"){
            Production p = tEntry.production;
            Symbol left = Symbol(p.left);
            //语义分析阶段
            if(p.right[0]=="E"){//E为空，不处理
            }else if(p.left == "simpleexpr"){
                if(p.right[0] == "ID"){
                    Symbol id = myStack.top().symbol;
                    myStack.pop();
                    // string val;
                    // if(id.map.find(id.entry)!=id.map.end()){
                    //     val = id.map[id.entry].value;
                    // }else{
                    //     val = idMap[id.entry].value;
                    // }
                    // left.val = val;
                    // left.map[id.entry] = val;
                    left.val = idMap[id.entry].value;
                    left.type = idMap[id.entry].type;
                }else if(p.right[0] == "NUM"){
                    Symbol inum = myStack.top().symbol;
                    myStack.pop();
                    left.val = inum.val;
                    left.type = inum.type;
                // }else if(p.right[0] == "REALNUM"){
                //     Symbol rnum = myStack.top().symbol;
                //     myStack.pop();
                //     left.val = rnum.val;
                //     left.type = rnum.type;
                }else if(p.right[0] == "("){ // ( arithexpr )
                    myStack.pop();
                    Symbol aexpr = myStack.top().symbol;
                    myStack.pop();
                    left.val = aexpr.val;
                    left.type = aexpr.type;
                    myStack.pop();
                }
            }else if(p.left == "multexprprime"){
                Symbol mul = myStack.top().symbol;
                myStack.pop();
                Symbol sp = myStack.top().symbol;
                myStack.pop();
                myStack.pop();
                if(p.right[0] == "*"){// * simpleexpr multexprprime
                    left.op = "*";
                    if(mul.op==""){
                        left.val = sp.val;
                        left.type = sp.type;
                    }else{
                        if(sp.type=="real" && mul.type=="real"){
                            left.type = "real";
                            if(left.op == mul.op){
                                left.val = to_string(atof(sp.val.c_str()) * atof(mul.val.c_str()));
                            }else{
                                left.val = to_string(atof(sp.val.c_str()) / atof(mul.val.c_str()));
                            }
                        }else if(sp.type=="real" && mul.type=="int"){
                            left.type = "real";
                            if(left.op == mul.op){
                                left.val = to_string(atof(sp.val.c_str()) * atoi(mul.val.c_str()));
                            }else{
                                left.val = to_string(atof(sp.val.c_str()) / atoi(mul.val.c_str()));
                            }
                        }else if(sp.type=="int" && mul.type=="real"){
                            left.type = "real";
                            if(left.op == mul.op){
                                left.val = to_string(atoi(sp.val.c_str()) * atof(mul.val.c_str()));
                            }else{
                                left.val = to_string(atoi(sp.val.c_str()) / atof(mul.val.c_str()));
                            }
                        }else{
                            left.type = "int";
                            if(left.op == mul.op){
                                left.val = to_string(atoi(sp.val.c_str()) * atoi(mul.val.c_str()));
                            }else{
                                left.val = to_string(atoi(sp.val.c_str()) / atoi(mul.val.c_str()));
                            }                        
                        }
                    }
                }else if(p.right[0] =="/"){// / simpleexpr multexprprime
                    left.op = "/";
                    if(is0(sp.val)){
                        errors.push_back(Error(4,line,""));//除0
                        left.stype = "error";
                        left.val = 1;
                        left.type = "int";
                    }else if(mul.op==""){
            ...
            }else if(p.left == "multexpr"){// simpleexpr multexprprime
                Symbol mul = myStack.top().symbol;
                myStack.pop();
                Symbol sp = myStack.top().symbol;
                myStack.pop();
            ...
            }else if(p.left == "arithexprprime"){              
                Symbol mul = myStack.top().symbol;
                myStack.pop();
                Symbol sp = myStack.top().symbol;
                myStack.pop();
                myStack.pop();
                if(p.right[0] == "+"){//  + multexpr arithexprprime
                    left.op = "+";
                    if(mul.op==""){
                        left.val = sp.val;
                        left.type = sp.type;
                ...
                }else if(p.right[0] =="-"){//  - multexpr arithexprprime
                    left.op = "-";
                    if(mul.op==""){
                        left.val = sp.val;
                        left.type = sp.type;
            ...
            }else if(p.left == "arithexpr"){// multexpr arithexprprime
                Symbol mul = myStack.top().symbol;
                myStack.pop();
                Symbol sp = myStack.top().symbol;
                myStack.pop();
            ...
            }else if(p.left == "boolexpr"){// arithexpr boolop arithexpr
                Symbol rop = myStack.top().symbol;
                myStack.pop();
                Symbol bop = myStack.top().symbol;
                myStack.pop();
                Symbol lop = myStack.top().symbol;
                myStack.pop();
                left.type = "bool";
                if(bop.op == "<"){
                    if(lop.val < rop.val){
            ...
            }else if(p.left == "assgstmt"){// ID = arithexpr ;
                myStack.pop();
                Symbol expr = myStack.top().symbol;
                myStack.pop();
                myStack.pop();
                Symbol id = myStack.top().symbol;
                myStack.pop();
                if(idMap[id.entry].type == "int" && expr.type =="real"){
                    errors.push_back(Error(5,line,""));
                    left.stype = "error";
                }
                if(isIf){
                    left.map[id.entry].value = expr.val;
                }else{
                    idMap[id.entry].value = expr.val;
                }
                // cout<<"-----------"<<id.entry<<"-----"<<idMap[id.entry].value<<endl;
            }else if(p.left == "ifstmt"){// if ( boolexpr ) then stmt else stmt
                Symbol stmt2 = myStack.top().symbol;
                myStack.pop();
                myStack.pop();
                Symbol stmt1 = myStack.top().symbol;
                myStack.pop();
                myStack.pop();
                myStack.pop();
                Symbol bexpr = myStack.top().symbol;
                myStack.pop();
                myStack.pop();
                myStack.pop();
                isIf = false;
                if(bexpr.val == "true"){
                    for(string v:variables){
                        if(stmt1.map.find(v)!=stmt1.map.end()){
                            idMap[v].value = stmt1.map[v].value;
                            // cout<<"-----------"<<v<<"-----"<<idMap[v].value<<endl;
                        }
                    }
                }else{
                    for(string v:variables){
                        if(stmt2.map.find(v)!=stmt2.map.end()){
                            idMap[v].value = stmt2.map[v].value;
                            // cout<<"-----------"<<v<<"-----"<<idMap[v].value<<endl;
                        }
                    }                    
                }  
            }else if(p.left == "stmts"){// stmt stmts
                Symbol stmts = myStack.top().symbol;
                myStack.pop();
                Symbol stmt = myStack.top().symbol;
                myStack.pop();
                if(!stmts.map.empty()){//不太对
                    for(string v:variables){
                        if(stmts.map.find(v)!=stmts.map.end()){
                            left.map[v].value = stmts.map[v].value;
                        }
                    } 
                }else{//不太对
                    for(string v:variables){
                        if(stmt.map.find(v)!=stmt.map.end()){
                            left.map[v].value = stmt.map[v].value;
                        }
                    } 
                }
            }else if(p.left == "compoundstmt"){// { stmt }
                myStack.pop(); 
                Symbol stmts = myStack.top().symbol;
                myStack.pop();   
                myStack.pop(); 
                for(string v:variables){//不太对
                    if(stmts.map.find(v)!=stmts.map.end()){
                        left.map[v].value = stmts.map[v].value;
                    }
                } 
                ...
                // if(p.right[0] == "ifstmt"){
                // }else if(p.right[0] == "assgstmt"){
                // }else if(p.right[0] == "compoundstmt"){
                // }
            }else if(p.left == "decl"){
                Symbol num = myStack.top().symbol;
                myStack.pop();  
                myStack.pop();  
                Symbol id = myStack.top().symbol;
                myStack.pop();   
                myStack.pop();     
                if(p.right[0] == "int"){ // int ID = INTNUM
                    if(num.type=="real"){
                        errors.push_back(Error(5,inputs[i-1].line,""));
                        left.stype = "error";
                    }
                    idMap[id.entry].type = "int";
                    idMap[id.entry].value = num.val;
                    // cout<<"-----------"<<id.entry<<"-----"<<idMap[id.entry].value<<endl;
                }else if(p.right[0] == "real"){// real ID = REALNUM
                    idMap[id.entry].type = "real";
                    idMap[id.entry].value = num.val;
                    // cout<<"-----------"<<id.entry<<"-----"<<idMap[id.entry].value<<endl;
                }
            }else if(p.left == "decls"){
                myStack.pop();   
                myStack.pop(); 
                myStack.pop(); 
                if(p.right[0] == "decl"){ // decl ; decls
                }
            }else if(p.left == "program"){ // decls compoundstmt
                myStack.pop(); 
                myStack.pop(); 
            }
            // for(int j=0;j<num;j++){//原来的LR
            //     myStack.pop();
            // }
            top = myStack.top();
            tEntry = table[top.stateId][p.left];
            if(tEntry.type=="goto"){
                myStack.push(SEntry(tEntry.stateId, left));//todo:压栈的symbol not string
            }else{
...
void printProcessState(SEntry sEntry, string input,TEntry tEntry){
    cout<<"stack state id:  "<<sEntry.stateId<<toStringSymbol(sEntry.symbol)<<", input: "<<input<<", action: "<<toStringTEntry(tEntry)<<endl;
}

string toStringSymbol(Symbol s){
    return "  symbol: "+s.symbol+", entry: "+s.entry+", type: "+s.type+", val: "+s.val+", stype: "+s.stype;
}
	...
	for(Error e:errors){
		...
		switch(e.id){
            ...
            case 4:
                type = "divide 0";
                if(ERROR){//普适输出
					error_msg = "line "+to_string(e.line)+", divide 0: ";
				}else{//OJ输出
					error_msg = "error message:line "+to_string(e.line)+",division by zero";
					cout<<error_msg<<endl;
				}
                break;
            case 5:
                type = "convert real to int";
                if(ERROR){//普适输出
					error_msg = "line "+to_string(e.line)+", convert real to int";	
				}else{//OJ输出
					error_msg = "error message:line "+to_string(e.line)+",realnum can not be translated into int type";
					cout<<error_msg<<endl;
				}
			default:
				type = "Error: ";
...
void printResults(){
    for(string v:variables){
        cout<<v<<": "<<standardValue(idMap[v].value)<<endl;
    }
}

string standardValue(string value){
    if(value.length()<=3){
        return value;
    }
    int i;
